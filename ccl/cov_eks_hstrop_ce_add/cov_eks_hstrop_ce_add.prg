...
drop program cov_eks_hstrop_ce_add:dba go
create program cov_eks_hstrop_ce_add:dba
...
with OUTDEV, TRIGGER, CE_ID, RESULT_VALUE, NORMALCY
...
set t_rec->result_value = $RESULT_VALUE
set t_rec->normalcy = $NORMALCY
...
head ce.clinical_event_id
	...
	case (t_rec->normalcy)
		of "RULED OUT": 		t_rec->qual[cnt].normalcy_ind = 0
		of "NO INJURY":			t_rec->qual[cnt].normalcy_ind = 0
		of "INDETERMINATE":		t_rec->qual[cnt].normalcy_ind = 1
		of "ABNORMAL":			t_rec->qual[cnt].normalcy_ind = 2
	else
		t_rec->qual[cnt].normalcy_ind = 2
	endcase
...
for (i = 1 to t_rec->cnt)
	...
	detail
		...
		EKSOPSRequest->qual[cnt].data[1].vc_var = t_rec->result_value
 		EKSOPSRequest->qual[cnt].data[1].long_var = t_rec->qual[i].normalcy_ind
 
 		if (EKSOPSRequest->qual[cnt].data[1].vc_var = "")
 			EKSOPSRequest->qual[cnt].data[1].vc_var = "ERROR"
 			EKSOPSRequest->qual[cnt].data[1].long_var = 2
 		endif
		...
		call writeLog(build2("---->EKSOPSRequest->qual[",trim(cnvtstring(cnt)),"].data[1].long_var=",
			trim(cnvtstring(EKSOPSRequest->qual[cnt].data[1].long_var))))
		call writeLog(build2("---->EKSOPSRequest->qual[",trim(cnvtstring(cnt)),"].data[1].vc_var=",
			trim((EKSOPSRequest->qual[cnt].data[1].vc_var))))
	with nocounter
	set dparam = 0
