DROP PROGRAM cov_mp_add_document :dba GO
CREATE PROGRAM cov_mp_add_document :dba
 PROMPT
  ...
  "Encounter Id:" = 0.0 ,
  "Event Code:" = 0.0 ,
  "Document Subject:" = 0.0 ,
  "Document Contents:" = 0.0 ,
  "Provider Patient Relationship Code:" = 0.0
  WITH outdev ,inputpersonid ,inputproviderid ,inputencounterid ,inputdoccode ,inputdocsubj ,
  inputdoccontent ,inputppr
 RECORD report_data (
   1 rep [* ]
     2 sb
       3 severitycd = i4
 ...
 SUBROUTINE  (validateuserprivileges (valuerec =vc (ref ) ,view_results_ind =i2 ,
  ...
  CALL log_message (build ("Exit ValidateUserPrivileges(), Elapsed time in seconds:" ,datetimediff (
     cnvtdatetime (sysdate ) ,begin_date_time ,5 ) ) ,log_level_debug )
 END ;Subroutine
 SET log_program_name = "MP_ADD_DOCUMENT"
 ...
 DECLARE action_type_sign = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,21 ,"SIGN" ) )
 ...
 DECLARE entry_mode_cd = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,29520 ,
   "WORKING_VIEW" ) )
 DECLARE event_reltn_root = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,24 ,"ROOT" ) )
 DECLARE event_reltn_child = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,24 ,"CHILD" ) )
 DECLARE succession_type_interim = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,63 ,
   "INTERIM" ) )
 DECLARE storage_blob = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,25 ,"BLOB" ) )
 ;DECLARE format_rtf = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,23 ,"RTF" ) )
 DECLARE format_rtf = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,23 ,"JSON" ) )
 DECLARE compression_no = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,120 ,"NOCOMP" ) )
 ...
 CALL getparametervalues (5 ,doccode_rec )
 CALL validaterequestrecord (null )
 CALL validateuserprivileges (doccode_rec ,privviewres ,privadddoc ,privmoddoc ,privunchartdoc ,
 ...
 SUBROUTINE  validaterequestrecord (null )
  ...
  ELSEIF (( $INPUTPROVIDERID = 0.0 ) )
   ...
   GO TO exit_script
  ELSEIF (( $INPUTDOCCODE = null ) )
   CALL handleerror ("ValidateRequestRecord()" ,"F" ,"" ,"Event Code cannot be null" ,report_data )
   GO TO exit_script
  ELSEIF (( $INPUTDOCCONTENT = null ) )
   CALL handleerror ("ValidateRequestRecord()" ,"F" ,"" ,"Document Content cannot be null" ,
    report_data )
   GO TO exit_script
  ENDIF
 ...
 SUBROUTINE  populatecesettings (null )
  ...
  SET ce_settings->event_cd =  $INPUTDOCCODE
  SET ce_settings->record_status_cd = active
  SET ce_settings->result_status_cd = auth_verified
  SET ce_settings->contributor_system_cd = powerchart
  SET ce_settings->event_class_cd = mdoc
 ...
 SUBROUTINE  insertceresults (null )
  ...
  SET srvstat = uar_srvsetstring (hreqlist ,"eso_action_meaning" ,nullterm ("ESOSKIP" ) )
  ...
  SET srvstat = uar_srvsetdouble (hstce ,"contributor_system_cd" ,ce_settings->contributor_system_cd
   )
  SET srvstat = uar_srvsetdouble (hstce ,"event_class_cd" ,ce_settings->event_class_cd )
  SET srvstat = uar_srvsetdouble (hstce ,"event_reltn_cd" ,event_reltn_root )
  SET srvstat = uar_srvsetdouble (hstce ,"event_cd" ,ce_settings->event_cd )
  SET srvstat = uar_srvsetdate (hstce ,"event_end_dt_tm" ,cnvtdatetime (ce_settings->event_end_dt_tm
    ) )
  SET srvstat = uar_srvsetdouble (hstce ,"task_assay_cd" ,ce_settings->task_assay_cd )
  ...
  SET srvstat = uar_srvsetdouble (hstce ,"entry_mode_cd" ,ce_settings->entry_mode_cd )
  SET srvstat = uar_srvsetstring (hstce ,"event_title_text" ,nullterm ( $INPUTDOCSUBJ ) )
  CALL addtoeventprsnllist (hstce ,ce_settings->person_id ,ce_settings->action_prsnl_id ,
   ...
   report_data )
  CALL addtoeventprsnllist (hstce ,ce_settings->person_id ,ce_settings->action_prsnl_id ,
   action_type_sign ,action_status_completed ,cnvtdatetime (ce_settings->action_dt_tm ) ,report_data
   )
  CALL addtoeventprsnllist (hstce ,ce_settings->person_id ,ce_settings->action_prsnl_id ,
   action_status_completed ,action_status_completed ,cnvtdatetime (ce_settings->action_dt_tm ) ,
   report_data )
  SET srvstat = uar_srvbinditemtype (hstce ,"child_event_list" ,uar_srvcreatetypefrom (hstce ,0 ) )
  SET hstchildce = uar_srvadditem (hstce ,"child_event_list" )
  IF (hstchildce )
   SET srvstat = uar_srvsetdouble (hstchildce ,"person_id" ,ce_settings->person_id )
   SET srvstat = uar_srvsetdouble (hstchildce ,"encntr_id" ,ce_settings->encntr_id )
   SET srvstat = uar_srvsetdouble (hstchildce ,"contributor_system_cd" ,powerchart )
   SET srvstat = uar_srvsetdouble (hstchildce ,"event_class_cd" ,doc )
   SET srvstat = uar_srvsetdouble (hstchildce ,"event_cd" ,ce_settings->event_cd )
   SET srvstat = uar_srvsetdouble (hstchildce ,"event_reltn_cd" ,event_reltn_child )
   SET srvstat = uar_srvsetdouble (hstchildce ,"performed_prsnl_id" ,ce_settings->action_prsnl_id )
   SET srvstat = uar_srvsetdouble (hstchildce ,"record_status_cd" ,ce_settings->record_status_cd )
   SET srvstat = uar_srvsetdouble (hstchildce ,"result_status_cd" ,ce_settings->result_status_cd )
   SET srvstat = uar_srvsetdouble (hstchildce ,"verified_prsnl_id" ,ce_settings->action_prsnl_id )
   SET srvstat = uar_srvsetdate (hstchildce ,"event_end_dt_tm" ,cnvtdatetime (ce_settings->
     event_end_dt_tm ) )
   SET srvstat = uar_srvsetdate (hstchildce ,"verified_dt_tm" ,cnvtdatetime (ce_settings->
     event_end_dt_tm ) )
   SET srvstat = uar_srvsetdate (hstchildce ,"performed_dt_tm" ,cnvtdatetime (ce_settings->
     event_end_dt_tm ) )
   SET srvstat = uar_srvsetdate (hstchildce ,"action_dt_tm" ,cnvtdatetime (ce_settings->action_dt_tm
     ) )
   SET srvstat = uar_srvsetshort (hstchildce ,"authentic_flag" ,1 )
   SET srvstat = uar_srvsetshort (hstchildce ,"publish_flag" ,1 )
   SET srvstat = uar_srvsetstring (hstchildce ,"collating_seq" ,nullterm ("1" ) )
   CALL addtoeventprsnllist (hstchildce ,ce_settings->person_id ,ce_settings->action_prsnl_id ,
    action_type_perform ,action_status_completed ,cnvtdatetime (ce_settings->action_dt_tm ) ,
    report_data )
   SET hblobres = uar_srvadditem (hstchildce ,"blob_result" )
   IF (hblobres )
    SET srvstat = uar_srvsetdouble (hblobres ,"succession_type_cd" ,succession_type_interim )
    SET srvstat = uar_srvsetdouble (hblobres ,"storage_cd" ,storage_blob )
    SET srvstat = uar_srvsetdouble (hblobres ,"format_cd" ,format_rtf )
    SET srvstat = uar_srvsetshort (hblobres ,"max_sequence_nbr" ,1 )
    SET srvstat = uar_srvsetshort (hblobres ,"valid_until_dt_tm_ind" ,1 )
    SET hblob = uar_srvadditem (hblobres ,"blob" )
    IF (hblob )
     SET srvstat = uar_srvsetdouble (hblob ,"compression_cd" ,compression_no )
     SET srvstat = uar_srvsetasis (hblob ,"blob_contents" , $INPUTDOCCONTENT ,size (
        $INPUTDOCCONTENT ) )
     SET srvstat = uar_srvsetlong (hblob ,"blob_length" ,size ( $INPUTDOCCONTENT ) )
    ENDIF
   ENDIF
  ENDIF
  SET iret = uar_crmperform (hstep )
  IF ((iret != 0 ) )
   CALL handleerror ("InsertCEResults()" ,"F" ,"Request Perform Failed" ,cnvtstring (iret ) ,
    report_data )
  ENDIF
  SET hreply = uar_crmgetreply (hstep )
 ...
 SUBROUTINE  (getcerep (hreply =i4 (ref ) ) =null WITH protect )
  ...
  IF ((replylistcnt > 0 ) )
   ...
   IF ((dynamiclabelcnt > 0 ) )
    SET stat = alterlist (report_data->dynamic_label_list ,5 )
