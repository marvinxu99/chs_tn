...
DROP PROGRAM cov_inqa_appt_index_past :dba GO
CREATE PROGRAM cov_inqa_appt_index_past :dba
...
call echo("starting query")
 ...
 SELECT
  ...
  HEAD a.sch_event_id
   ;start 001
   call echo("-->starting 001 facility logic")
   facility_pos = 0
   sch_location_pos = 0
   skip_ind = 1
   if (e1.encntr_id > 0.0)
   	call echo("--->inside e1.encntr_id > 0.0")
   ...
   elseif (e1.encntr_id = 0.0)
    call echo("--->inside e1.encntr_id = 0.0")
   	...
   	call echo(build2("---->sch_location_pos=",trim(cnvtstring(sch_location_pos))))
   	if (sch_location_pos > 0)
   	 call echo(build2("----->inside sch_location_pos > 0"))
   	 facility_pos = 
   	 ...
   	 if (facility_pos > 0)
   	  call echo(build2("----->inside facility_pos > 0"))
   	  skip_ind = 0
   	 endif
   	endif
   endif	
   call echo("<--ending 001 facility logic")				
;end 001
   ...
   .disp_display ,reply->query_qual[reply->query_qual_cnt ].location = concat(trim(ed1.disp_display))
  endif				;001
  FOOT REPORT
