...
******************************************************************************
	...
	Date Written:
	Solution:
	Source file name:	cov_consult_order_pod.prg
	Object name:		cov_consult_order_pod
	Request #:
...
drop program cov_consult_order_pod:dba go
create program cov_consult_order_pod:dba
...
join oc
	...
	and   oc.active_ind = 1
	and   oc.primary_mnemonic = "Consult to Podiatry"
order by
	 oc.primary_mnemonic
	,oc.catalog_cd
head report
...
join cv1
	...
	and   cv1.display in ("Podiatry Consultation","Orthopedic Consultation","Surgical Consultation")
join p
...
head encntr_id
	null
head order_id
	if (cv1.display = "Surgical Consultation")
		t_rec->qual[d2.seq].surg_event_id = ce.event_id
		t_rec->qual[d2.seq].surg_note_dt_tm = ce.event_end_dt_tm
		t_rec->qual[d2.seq].surg_note_author = p.name_full_formatted
		t_rec->qual[d2.seq].surg_note_type = uar_get_code_display(ce.event_cd)
	else
		t_rec->qual[d2.seq].event_id = ce.event_id
		t_rec->qual[d2.seq].note_dt_tm = ce.event_end_dt_tm
		t_rec->qual[d2.seq].note_author = p.name_full_formatted
		t_rec->qual[d2.seq].note_type = uar_get_code_display(ce.event_cd)
	endif
with nocounter
...
if (t_rec->prompts.report_type = 1)
	...
	select into t_rec->prompts.outdev
		...
		,note_date = format(t_rec->qual[d1.seq].note_dt_tm,"dd-mmm-yyyy hh:mm:ss;;d")
		,note_author = substring(1,100,t_rec->qual[d1.seq].note_author)
		,note_type = substring(1,100,t_rec->qual[d1.seq].note_type)
		,surg_note_date = format(t_rec->qual[d1.seq].surg_note_dt_tm,"dd-mmm-yyyy hh:mm:ss;;d")
		,surg_note_author = substring(1,100,t_rec->qual[d1.seq].surg_note_author)
		,surg_note_type = substring(1,100,t_rec->qual[d1.seq].surg_note_type)
		,order_id=t_rec->qual[d1.seq].order_id
		,encntr_id=t_rec->qual[d1.seq].encntr_id
		,event_id=t_rec->qual[d1.seq].event_id
		,surg_event_id=t_rec->qual[d1.seq].surg_event_id
	from
