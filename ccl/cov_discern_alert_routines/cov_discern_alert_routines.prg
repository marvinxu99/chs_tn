...
subroutine  sGetAllPatientDiscernAlert (vEncntrID,vPersonLevel,vHistorical )
	...
	head ce.event_id
	     ...
	     discern_alerts->qual[i].valid_from_dt_tm = ce.valid_from_dt_tm
	     discern_alerts->qual[i].valid_until_dt_tm = ce.valid_until_dt_tm
	     discern_alerts->qual[i].encntr_id = e.encntr_id
	     discern_alerts->qual[i].person_id = p.person_id
...
/**********************************************************************************************************************
** Function sGetAllDiscernAlerts(vHistorical)
** ---------------------------------------------------------------------------------------
** Load all the active alerts
**********************************************************************************************************************/
declare sGetAllDiscernAlerts(vHistorical=i2(VALUE,0)) = vc with copy, persist
subroutine  sGetAllDiscernAlerts (vHistorical )
	
	call SubroutineLog(build2('start sGetAllDiscernAlerts(',vHistorical,')'))	
