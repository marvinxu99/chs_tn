...
******************************************************************************
  ...
  Date Written:      
  Solution:
  Source file name:   cov_cnt_rl_baby_mom_link.prg
  Object name:        cov_cnt_rl_baby_mom_link
  Request #:
  ...
  Special Notes:      copied from lh_cnt_rl_mom_baby_link and reversed to cov_cnt_rl_baby_mom_link
...
******************************************************************************/
DROP PROGRAM cov_cnt_rl_baby_mom_link :dba GO
CREATE PROGRAM cov_cnt_rl_baby_mom_link :dba
 ...
 select into "nl:"
	...
	join ppr
		...
		and   ppr.person_reltn_cd in(value(uar_get_code_by("DISPLAYKEY",40,"CHILD")))
	join p2
		where p2.person_id = ppr.related_person_id
	join e
		where e.person_id = p2.person_id
		and   e.encntr_type_cd in(value(uar_get_code_by("DISPLAYKEY",71,"NEWBORN")))
	order by
		e.encntr_id
		,e.reg_dt_tm desc
  head e.encntr_id
   ...
   CALL echo (build (" Baby person id   " ,mompersonid ) ) ,
   CALL echo (build (" Baby encntr id   " ,momencntrid ) )
  WITH nocounter
 ;end select
