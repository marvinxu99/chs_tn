...
******************************************************************************
	...
	Solution:
	Source file name:	cov_pha_ops_prev_day.prg
	Object name:		cov_pha_ops_prev_day
	Request #:
...
drop program cov_pha_ops_prev_day:dba go
create program cov_pha_ops_prev_day:dba
...
set t_rec->reports[k].start_dt_tm		= t_rec->cons.dates.prev_day_start_dt_tm
set t_rec->reports[k].end_dt_tm			= t_rec->cons.dates.prev_day_end_dt_tm
...
for (i=1 to t_rec->cons.acute_facility_cnt)
	...
	set t_rec->reports[k].instances[j].filename		= build(
																...
																,cnvtlower("COV_PHQ_BCMA_LEAPFROG_DAILY")
																;,"_"
