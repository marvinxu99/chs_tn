   ...
   Select into value($OUTDEV)
	...
	Head report
		file_header_var = build(
			...
			,wrap3("Pharmacy_ID")
			,wrap3("Pharmacy_name")
			,wrap3("Pharmacy_Address")
			,wrap3("Pharmacy_State")
			,wrap3("Pharmacy_City")
			,wrap3("Pharmacy_Zip")
			,wrap3("Pharmacy_NPI")
			,wrap3("Pharmacy_NCPDP")
			,wrap3("status")
  			,wrap3("Strength Dose")
	...
	Head d.seq
		...
		output_orders = build(output_orders
			...
			,wrap3(rx->olist[d.seq].ROUTINGPHARMACYID)
			,wrap3(rx->olist[d.seq].pharmacy_name)
			,wrap3(rx->olist[d.seq].Address)
			,wrap3(rx->olist[d.seq].state)
			,wrap3(rx->olist[d.seq].city)
			,wrap3(rx->olist[d.seq].zipcode)
			,wrap3(rx->olist[d.seq].npi)
			,wrap3(rx->olist[d.seq].NCPDP)
